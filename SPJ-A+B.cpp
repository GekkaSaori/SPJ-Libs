...
int main(int argc,char *argv[])
{
    ...
    double userans=readuserdouble();
    double stdans=readstddouble();
    detail("Your Answer: %f\nStandard: %f\n",userans,stdans);
    if(fabs(stdans-userans)<=1e-9)give(AC),log("You got the correct answer.");
    else give(5),log("You didn\'t got the correct answer.");
    close();
    return 0;
}
